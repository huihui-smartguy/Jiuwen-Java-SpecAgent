...
| `--fault-lib` | 故障库（规格库）路径，供 stage2.6 匹配（默认自动探测 `TestKnowledgeBase/Fault/rest_api_faults.json`；`Specification_Repository` 仅遗留兼容） |
...
> **是什么**：故障库（规格库）是一份**外部可插拔**的缺陷经验知识源。后续迭代以 `TestKnowledgeBase/Fault/*.json` 为知识库主干；`Specification_Repository/` 不再承担实际产品迭代职责，仅作为既有示例和旧调用的兼容来源。流水线判据（Oracle）来自"需求 + 真实契约"，能保证"形态正确、流程跑通"，但缺少"**应该测哪些失败模式**"。故障库正是补这块——经**文件注入**，**不硬编码**进 Skill 提示词。

### 7.1 默认即启用（零配置）

标准模式下 `--faults auto`（默认）会**优先自动探测** `TestKnowledgeBase/Fault/rest_api_faults.json`；仅在该文件不存在时才回退到旧 `Specification_Repository/rest_api_common_faults.json`。探到后在 **stage2.6** 把故障按四类触发（端点/方法、标签、历史缺陷、关联字段）匹配进流水线，并用 `contract.md` 的字段权威性分级对每条故障的断言级别做**契约优先封顶**（spec-required→至多 L2；config-dependent/契约静默/未知→降 L0/L1）。
...
| 诉求 | 做法 |
|------|------|
| 默认启用 | 标准调用即可（优先探测 `TestKnowledgeBase/Fault/`） |
...
```bash
# 默认（自动探测 TestKnowledgeBase/Fault/）
/auto-test-flow 需求.md <java仓>/<模块> --sut-base-url http://host:port
...
| **人工裁决** | 故障库（规格库）已接入，后续默认知识源为 `TestKnowledgeBase/Fault/`；stage3b 的故障覆盖完备性维度可由故障库背书自动通过，stage1 与 FP 拆分/断言合理性仍人工 |
