...
remediation.config.json                      # 修复配置：upstream=fake_repo, reverify.simulate=true, switches.allow_*=false
...
.state/remediation/defects/<case>/           # 模拟 stage6 子 Agent 产出（golden 输入）：
                                             #   patch.diff(业务) / regression_test.diff(自测) / issue.md / pr.md / root_cause.md / confidence.json
```

## 复现命令（在仓库根）

```bash
# 1) 深析前处理：sdk_defect → worklist
python AutoTestFlow/scripts/remediation_plan.py --output-dir AutoTestFlow/examples/a2a/remediation_demo

#   （真实流程：编排器据 plan.json 对每个缺陷启动 stage6_defect_analyze 子 Agent 产出 defects/<case>/*；
...
- `.state/remediation/plan.json` —— 2 条 sdk_defect worklist（TC_041 SPEC-RESP-WRAP / TC_042 SPEC-ID-TYPE）。
...
- `.state/remediation/submitted.json` —— `mode=dry-run`，push/PR/issue 全跳过（**零 gh 调用**）。
...
- **第二层保险**：`--remediate on --gate-confirmed` 但配置 `switches.allow_*=false` → 仍**零 gh 调用**，
  `submitted.json` 记 push/pr/issue 三跳过。
- **绿前提红线**：把 `switches.require_green_before_pr` 改成 `false` → 配置加载即被拒绝（ConfigError）。
...
> `gh` 开 PR/issue）仅在用户用真实仓 + 构建工具链 + `gh` 鉴权运行 `--remediate=on/dry-run` 时走通；
