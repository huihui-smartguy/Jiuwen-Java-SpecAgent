...
- **物理隔离**：Beta 代码全部在 Worker-local 子树内：
  `AutoTestFlow/workers/Stage26-KnowledgeMatch/` 与
  `AutoTestFlow/workers/Stage6-FaultAnalysis/`。
...
- **不破坏内核**：Beta 特性**不得违反** `DESIGN.md` §1–§7（尤其 §2「contract 唯一 Oracle / 生成器不得自我认证」、
  §7「不洗绿」）与 `workers/_common/shared/rules.md` §4（5 分类执行边界）。Beta 只能在这些不变量之上做**增强**，不能绕过它们。
...
```
AutoTestFlow/workers/Stage26-KnowledgeMatch/
├── beta/README.md                         # 本文件
├── scripts/
...
AutoTestFlow/workers/Stage6-FaultAnalysis/templates/
...
python AutoTestFlow/workers/Stage26-KnowledgeMatch/scripts/gen_wiki.py

# 2) 护栏校验（全绿才算 wiki 可用）
python AutoTestFlow/workers/Stage26-KnowledgeMatch/scripts/check_wiki.py        # 产 .state/wiki_check.json，全绿退出 0
...
- **复现离线 demo**：见 `AutoTestFlow/workers/Stage26-KnowledgeMatch/examples/a2a/wiki_demo/README.md`（无需 SUT/LLM）。
...
TestKnowledgeBase/Fault/rest_api_faults.json           ← 后续单一真相（matcher/record 默认输入）
   │
   ├──► workers/Stage26-KnowledgeMatch/scripts/match_faults.py / workers/Stage5-Report/scripts/record_faults.py
   │      [确定性消费者：只吃 JSON，零改动]
   │
   └──(workers/Stage26-KnowledgeMatch/scripts/gen_wiki.py 单向派生)──► TestKnowledgeBase/Fault/wiki/*.md（advisory NL 层）
          ...
          └──► stage2.6b(beta) / stage6(beta) 的 LLM 子 Agent   [LLM 消费者：按 fault_id 读 wiki]
                 判据仍由 contract.md 封顶；wiki 不作 oracle
                              ▲
            workers/Stage26-KnowledgeMatch/scripts/check_wiki.py 把上述护栏变为可执行断言（离线机械校验）
```

## 六、毕业 / 退役标准

- **毕业到稳定**：当某 Beta 特性经多 SUT 验证稳定、文档/测试齐备、无内核冲突 → 把 Worker-local beta 资产并入对应 Worker 的稳定资产，
  开关从 `--beta-*` 提升为正式参数，更新 `DESIGN.md`/`SKILL.md`/ChangeLog。
- **退役**：若预研结论为不采用 → 删除对应 Worker-local beta 子项与开关，`TestKnowledgeBase/Fault/wiki/` 等派生产物一并移除；
