...
铁律（详见 workers/Stage26-KnowledgeMatch/shared/wiki_rules.md，呼应 DESIGN §2/§7 与 07 §六护栏）：
...
# ---------------- 通用 IO（与 workers/Stage26-KnowledgeMatch/scripts/match_faults.py 保持一致） ----------------
...
#      下列三函数与 workers/Stage26-KnowledgeMatch/scripts/match_faults.py 的同名实现逐字对齐，行为一致。
...
def render_fault_article(fault: dict, siblings: list, history_refs: list) -> str:
    ...
    lines.append("> 本文由 `workers/Stage26-KnowledgeMatch/scripts/gen_wiki.py` 从结构化故障库**单向派生**，仅作 advisory；"
...
def render_category_article(cat_key: str, cat_label: str, faults: list) -> str:
    ...
    lines.append("")
    lines.append(f"> 由 `workers/Stage26-KnowledgeMatch/scripts/gen_wiki.py` 从结构化故障库单向派生。本类共 {len(faults)} 条故障。")
...
def render_index(cat_groups: list, lib_meta: dict, overlay_meta) -> str:
    ...
    lines.append("")
    lines.append("> 本目录由 `AutoTestFlow/workers/Stage26-KnowledgeMatch/scripts/gen_wiki.py` 从 "
    ...
    lines.append("> 重生成：`python AutoTestFlow/workers/Stage26-KnowledgeMatch/scripts/gen_wiki.py`；校验：`python AutoTestFlow/workers/Stage26-KnowledgeMatch/scripts/check_wiki.py`。")
    lines.append("")
...
def auto_detect_lib(explicit: str) -> str:
    ...
    repo_root = here
    while True:
        if os.path.isdir(os.path.join(repo_root, "TestKnowledgeBase")):
            break
