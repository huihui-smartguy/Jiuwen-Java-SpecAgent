...
> 对应 `ChangeLogs/v3.0` §七验证、`AutoTestFlow/workers/Stage26-KnowledgeMatch/shared/wiki_rules.md` 护栏、`AutoTestFlow/workers/Stage26-KnowledgeMatch/shared/wiki_schema.md` 格式契约。
...
```bash
DEMO=AutoTestFlow/workers/Stage26-KnowledgeMatch/examples/a2a/wiki_demo

# 1) 由 sample 库单向派生 wiki 到临时目录
python3 AutoTestFlow/workers/Stage26-KnowledgeMatch/scripts/gen_wiki.py \
    --fault-lib $DEMO/faults.sample.json --wiki-dir /tmp/wiki_demo_out/wiki

# 2) 护栏校验（全绿，退出 0）
python3 AutoTestFlow/workers/Stage26-KnowledgeMatch/scripts/check_wiki.py \
