...
> 本目录由 `AutoTestFlow/workers/Stage26-KnowledgeMatch/scripts/gen_wiki.py` 从 `TestKnowledgeBase/Fault/rest_api_faults.json` [+ overlay] **单向派生**。
> **单一真相是结构化 JSON**；本目录仅作 advisory NL 层，供 stage2.6b / stage6 的 LLM 子 Agent 按 `fault_id` 确定性取用。**不作 oracle、不进 `match_faults.py`**。
> 重生成：`python AutoTestFlow/workers/Stage26-KnowledgeMatch/scripts/gen_wiki.py`；校验：`python AutoTestFlow/workers/Stage26-KnowledgeMatch/scripts/check_wiki.py`。
