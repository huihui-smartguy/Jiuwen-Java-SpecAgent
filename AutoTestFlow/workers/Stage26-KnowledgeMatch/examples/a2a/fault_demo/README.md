...
```bash
# 1) 阶段2.6 故障匹配（契约优先封顶）
python AutoTestFlow/workers/Stage26-KnowledgeMatch/scripts/match_faults.py \
    --output-dir AutoTestFlow/workers/Stage26-KnowledgeMatch/examples/a2a/fault_demo \
    --fault-lib Specification_Repository/rest_api_common_faults.json

# 2) 结果聚合（5 分类计数）
python AutoTestFlow/workers/Stage4-TestGenerationRun/scripts/aggregate_results.py \
    --output-dir AutoTestFlow/workers/Stage26-KnowledgeMatch/examples/a2a/fault_demo

# 3) 闭环自积累（dry-run；加 --write 落 overlay）
python AutoTestFlow/workers/Stage5-Report/scripts/record_faults.py \
    --output-dir AutoTestFlow/workers/Stage26-KnowledgeMatch/examples/a2a/fault_demo \
    --fault-lib Specification_Repository/rest_api_common_faults.json \
    --overlay-path AutoTestFlow/workers/Stage26-KnowledgeMatch/examples/a2a/fault_demo/project_faults.json --write

# 降级回归：关闭即不产出任何文件，与未接入时一致
python AutoTestFlow/workers/Stage26-KnowledgeMatch/scripts/match_faults.py \
    --output-dir AutoTestFlow/workers/Stage26-KnowledgeMatch/examples/a2a/fault_demo --faults off
```
