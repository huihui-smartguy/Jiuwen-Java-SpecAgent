...
> 每个用例的 oracle 必须引用 contract.md 的 specId，断言层级遵循 workers/_common/shared/rules.md。
...
- case_id起始: {next_seq}
- 输出路径: {output_dir}/TestCases/test_design_batch_{batch_id}.json

## 第零步：读取契约与规则（强制，先于设计）

1. Read `{output_dir}/Contract/contract.md` — **唯一权威契约**。提取每个 specId（SPEC-RESP-WRAP / SPEC-ID-TYPE / SPEC-ENUM / SPEC-SSE / SPEC-ERR-* / SPEC-CARD 等）及其 spec-required vs deployment-config-dependent 标注。
2. Read `{skill_dir}/workers/_common/shared/rules.md` — 断言层级（L0/L1/L2）与设计规则。
...
读取 `{output_dir}/FeatureAnalysis/stage_summary.json`，如果含 `user_test_entry` 且 `confidence` 为 high/medium：
...
如果 `{output_dir}/QualityGates/professional_case_guidance.json` 存在，读取它并使用其中的 `acceptance_refs` 与 `evidence_required`：
...
Read `{output_dir}/KnowledgeBase/fault_matches.json`。对本批每个 `source_scene`，取 `fault_matches` 中 `target_scenes` 含该场景的条目，逐条**额外生成一条故障导向用例**：
...
- 新增字段 **`fault_oracles`** = 匹配项 `fault_oracles`，必须原样保留。每个 `fault_ref` 用例必须至少包含 1 条 `required=true` 且 `kind=process` 或 `kind=negative` 的机器可检查 oracle；缺失时视为**设计错误**，不得输出为可通过用例。
- `priority` 取匹配项 `priority`（历史缺陷为 P0）。
- `steps` 用用户操作语言：基于场景正常步骤，在触发步骤注入匹配项 `trigger`（如 `metadata.agentId='nonexistent-<uuid>'`）；`expected` = 匹配项 `expected_behavior_raw` + 由 `oracle_refs` 和 `fault_oracles` 派生的验证点（≥2维度、≥1条 spec-required L2、≥1条过程/否定 oracle）。
- 数量：故障用例数已由 2.6 按类配额限制，此处**不再额外截断**，全部生成。

故障导向用例与既有 case_kind 用例**同构**（同一 JSON schema，仅多 `fault_ref` / `fault_oracles` 字段），一并进入第四步输出与后续 merge。

> 注：若启用了阶段2.6b 增强，`fault_matches.json` 条目可能已被精化（补绑 `oracle_refs`、替换占位）；
> 对 `reconciliation:"contract_conflict"` 的条目按**观察用例**处理（记录差异、不作硬断言）；若其 `fault_oracles` 仍有 required 过程/否定项，stage4 不得仅凭最终响应成功写 pass。
...
[{
  ...
  "fault_ref": "F-REQ-011",
  "fault_oracles": [
    {"id": "F-REQ-011:no_unexpected_5xx:1", "kind": "negative", "check": "no_unexpected_5xx", "required": true, "authority": "fault-required"},
    {"id": "F-REQ-011:correlation_id_preserved:2", "kind": "process", "check": "correlation_id_preserved", "required": true, "authority": "spec-required", "spec_id": "SPEC-ID-TYPE"}
  ],
  "acceptance_refs": [
...
- **`test_type`（必填，测试维度）**：默认 `"scenario"`（场景维度，已实现，进入 stage4 执行）；`"dfx"` 为**规划占位**（Design for X：可靠性/性能/安全等非功能维度，仅登记、不生成可执行代码、由编排器跳过 stage4）。继承自来源场景的 test_type，场景未标注时默认 `scenario`。详见 workers/_common/shared/scenario_schema.md。
...
- 故障导向用例不能只把异常输入跑通后视为成功；`fault_oracles` 中的 required 过程/否定 oracle 将由 stage4 的 trace evaluator 决定能否 pass。
- steps 和 expected 必须是**纯文本字符串**。
- expected ≥ 2验证维度，且必须包含 ≥1个 L2 断言（层级定义见 workers/_common/shared/rules.md）。

**断言层级要求**（详见 workers/_common/shared/rules.md）：
...
Write `{output_dir}/TestCases/test_design_batch_{batch_id}.json`
...
3. 处理用户反馈：
   ...
   - 用户**提出修正** → 按反馈回改 `TestCases/test_design_batch_{batch_id}.json` 中对应用例，必要时重跑相关自检，再次确认。
