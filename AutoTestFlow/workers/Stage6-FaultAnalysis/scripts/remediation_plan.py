...
COMMON_SCRIPTS_DIR = os.path.join(WORKERS_DIR, "_common", "scripts")
COMMON_REFERENCE_DIR = os.path.join(WORKERS_DIR, "_common", "reference")
for path in (SCRIPT_DIR, COMMON_SCRIPTS_DIR, COMMON_REFERENCE_DIR):
...
def _repo_root_from(start):
    ...
    while True:
        if os.path.isdir(os.path.join(cur, "TestKnowledgeBase")):
            return cur
...
REPO_ROOT = _repo_root_from(SCRIPT_DIR)
