...
必须读取以下文件：
1. `{skill_dir}/workers/_common/shared/scenario_schema.md` — 统一场景JSON schema（含 framework_scenes schema）
...
格式遵循已读取的 `workers/_common/shared/scenario_schema.md`。
...
```bash
python {skill_dir}/workers/_common/scripts/render_design_markdown.py --output-dir {output_dir} --stage s3a
```
