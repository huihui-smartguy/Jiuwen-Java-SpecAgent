...
必须读取以下文件：
1. `{skill_dir}/workers/_common/shared/scenario_schema.md` — 统一场景JSON schema
...
**steps 构建规则**：
- 从 related_methods 构建完整用户操作链路：前置准备 → 调用主入口端点 → 使用相关端点 → 验证 → 收尾
...
**branches 补充规则**：
- exception：从 exception_catalog 查找 reachable_from 包含该能力入口的异常（保留 error_code）
