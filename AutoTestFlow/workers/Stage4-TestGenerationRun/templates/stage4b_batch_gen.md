...
> 协议特化解包（如 A2A 的 result.task）以 contract.md 路径为准，示例见 workers/Stage4-TestGenerationRun/examples/a2a/；本阶段一律用通用 `http_client` 原语 + 通用 helper。
...
3. `{skill_dir}/workers/Stage4-TestGenerationRun/reference/http_client.py` — 通用客户端 `HttpClient`（get/post_json/stream/raw_post）+ 容差 helper（id_eq/normalize_state/parse_sse_lines）。
