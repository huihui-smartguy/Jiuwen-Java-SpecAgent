...
2. `{skill_dir}/workers/Stage4-TestGenerationRun/reference/http_client.py` — 通用黑盒 HTTP 客户端 `HttpClient`（原语：`get` / `post_json` / `stream` / `raw_post`）+ 协议无关容差 helper（`id_eq` / `normalize_state` / `parse_sse_lines`）+ 交互轨迹记录器（仅可使用其列出的方法）。
...
> 协议特化的请求体构造与响应字段访问（如 A2A 的 result.task 解包、事件 oneof 解析）以 contract.md specId 为准；具体用法示例见 workers/Stage4-TestGenerationRun/examples/a2a/，本阶段一律用通用 `http_client` 原语 + 通用 helper。
...
- `{skill_dir}/workers/Stage4-TestGenerationRun/reference/http_client.py` → `{target_output_dir}/TestRun/http_client.py`
- `{skill_dir}/workers/Stage4-TestGenerationRun/reference/conftest.py` → `{target_output_dir}/TestRun/conftest.py`

```bash
mkdir -p {target_output_dir}/TestRun/tests {target_output_dir}/TestRun/results {target_output_dir}/TestRun/trace
cp -n {skill_dir}/workers/Stage4-TestGenerationRun/reference/http_client.py {target_output_dir}/TestRun/http_client.py
cp -n {skill_dir}/workers/Stage4-TestGenerationRun/reference/conftest.py {target_output_dir}/TestRun/conftest.py
```
...
```bash
python {skill_dir}/workers/Stage4-TestGenerationRun/scripts/evaluate_fault_oracles.py --output-dir {target_output_dir} --case-id {case_id} --write
```
