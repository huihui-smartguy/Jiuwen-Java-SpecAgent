...
`TestCases/test_examples.md` 是 `TestCases/test_design.json` 的确定性 Markdown 投影。所有 Markdown companion 均由 `workers/_common/scripts/render_design_markdown.py` 从 JSON 渲染，保留 JSON provenance、`test_suggestion_refs`、`oracle_refs`、`fault_ref`、`fault_oracles`、`acceptance_refs` 等追踪字段；JSON 仍是唯一机器协议与权威数据源。
...
派生方法学见 `workers/Stage2-CodeAnalysisContract/shared/code_scan_guide.md`；Java/Spring 细节见 `workers/Stage2-CodeAnalysisContract/shared/java_scan_guide.md` profile 附录。
