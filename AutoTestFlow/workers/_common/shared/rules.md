...
> 本文件定义本 skill 所有阶段必须遵守的共用纪律，适用于**任意对外暴露 HTTP/SSE/RPC 契约的源码型 SUT**。
...
3. **交互轨迹强制采集** —— 记录器已内置于 `workers/Stage4-TestGenerationRun/reference/http_client.py`，每条用例必须产出可读请求/响应/SSE 轨迹。
...
> 断言所用的字段路径/枚举/错误码一律以 `contract.md` 为准（见 §2），通用容差访问器见 `workers/Stage4-TestGenerationRun/reference/http_client.py`。
...
- Stage4 在 pytest 通过后必须运行 `workers/Stage4-TestGenerationRun/scripts/evaluate_fault_oracles.py`。只有 `fault_oracle_summary.classification=="passed"` 时，fault case 才能保持 `status=passed`。
...
| 裸字段访问 SUT 响应（有容差时） | 用容差访问器（通用见 `workers/Stage4-TestGenerationRun/reference/http_client.py`） |
