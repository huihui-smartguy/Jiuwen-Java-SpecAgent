...
WORKERS_DIR = SCRIPT_DIR.parents[1]
COMMON_SCRIPTS_DIR = WORKERS_DIR / "_common" / "scripts"
for path in (SCRIPT_DIR, COMMON_SCRIPTS_DIR):
    path_str = str(path)
...
def build_plan(code_path: str | Path, output_dir: str | Path, profiles_path: str | Path | None = None) -> dict[str, Any]:
    ...
    plan = {
        "meta": {
            ...
            "code_path": str(root),
            "output_dir": str(out),
            "profile_catalog": "AutoTestFlow/workers/Stage2-CodeAnalysisContract/shared/code_scan_profiles.json",
        },
