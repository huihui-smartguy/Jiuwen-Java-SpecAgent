...
所有探针经 `--base-url`（或环境变量 BASE_URL）参数化；多 SUT 模式下可通过
`--target-id` 与 `--probe-plan` 写入目标身份和目标专属探针。
...
用法：
  python3 probe_contract.py --target-id api --base-url http://localhost:8080 --output contract_samples.json
  python3 probe_contract.py --probe-plan probes.json --dry-run
...
def load_probe_plan(path):
    """Load a manifest-provided probe plan from JSON.

    The JSON may be either a list of probe objects or a mapping with a `probes`
    list. Each probe follows the existing shape: name/method/path/accept/body/
    stream/note.
    """
    if not path:
        return None
    with open(path, "r", encoding="utf-8") as fh:
        doc = json.load(fh)
    probes = doc.get("probes") if isinstance(doc, dict) else doc
    if not isinstance(probes, list) or not probes:
        raise ValueError("--probe-plan must contain a non-empty probe list")
    out = []
    for idx, probe_item in enumerate(probes):
        if not isinstance(probe_item, dict):
            raise ValueError("probe %s must be an object" % idx)
        item = dict(probe_item)
        item.setdefault("name", "probe_%s" % idx)
        item.setdefault("method", "GET")
        item.setdefault("path", "/")
        item.setdefault("accept", "application/json")
        item.setdefault("stream", False)
        out.append(item)
    return out


def _resolve_probe_plan(base_url, probe_plan=None):
    return probe_plan if probe_plan is not None else _default_probe_plan(base_url)
...
# 真实探测（配置驱动：遍历探针计划）
# ---------------------------------------------------------------------------
def probe(base_url, timeout, probe_plan=None):
    samples = []
    notes = []
    base = base_url.rstrip("/")

    for p in _resolve_probe_plan(base, probe_plan):
        name = p["name"]
...
# dry-run 骨架
# ---------------------------------------------------------------------------
def build_dry_run(base_url, target_id="default", probe_plan=None, probe_plan_source="default:a2a-example"):
    plan = _resolve_probe_plan(base_url, probe_plan)
    ...
    return {
        ...
        "reachable": None,
        "samples": samples,
        "notes": ["dry_run：未发起任何网络请求；以上为探针计划骨架（%s）。"
                  "正式校准请去掉 --dry-run 并确保 SUT 在 %s 就绪。"
                  % (probe_plan_source, base_url)],
    }
...
def main(argv=None):
    ...
    parser.add_argument("--target-id", default=os.environ.get("AUTOTESTFLOW_TARGET_ID", "default"),
                        help="被测目标 id（多 SUT 模式下写入 contract_samples.json）")
    parser.add_argument("--probe-plan",
                        help="目标专属探针计划 JSON（list 或 {probes:[...]}; 缺省使用 A2A 示例探针）")
    ...
    try:
        probe_plan = load_probe_plan(args.probe_plan) if args.probe_plan else None
    except Exception as e:
        print("[probe_contract] probe plan 无效: %r" % e, file=sys.stderr)
        return 2
    probe_plan_source = os.path.abspath(args.probe_plan) if args.probe_plan else "default:a2a-example"

    # ---- dry-run：无网络、无 httpx ----
    if args.dry_run:
        result = build_dry_run(base_url, args.target_id, probe_plan, probe_plan_source)
        print("[probe_contract] DRY-RUN —— target=%s 探针计划（%s）：" % (
            args.target_id, probe_plan_source))
        for p in _resolve_probe_plan(base_url, probe_plan):
            print("  - %-16s %s %s%s" % (
                p["name"], p.get("method", ""),
                base_url, p.get("path", "")))
    ...
    samples, notes = probe(base_url, args.timeout, probe_plan)
    result = {
        "status": "probed",
        "target_id": args.target_id,
        "base_url": base_url,
