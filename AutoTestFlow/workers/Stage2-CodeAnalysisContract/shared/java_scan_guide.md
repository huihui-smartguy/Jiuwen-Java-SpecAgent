...
> 本文件是 `workers/Stage2-CodeAnalysisContract/shared/code_scan_guide.md` 的 `java.spring` profile 附录，指导 stage2 子 Agent 如何**静态扫描一个 Java/Spring 被测服务（SUT）**，
...
> （schema 见 `workers/_common/shared/scenario_schema.md` 的 framework_scenes）。这是 stage3a-fw 子Agent 消费的内部产物，
...
- 产出：`code_analysis.md`（格式见 `workers/Stage2-CodeAnalysisContract/shared/code_analysis_template.md`）+ `FeatureAnalysis/s2_code_facts.json` + `FeatureAnalysis/framework_scenes.json`（框架 E2E 场景派生物）。
