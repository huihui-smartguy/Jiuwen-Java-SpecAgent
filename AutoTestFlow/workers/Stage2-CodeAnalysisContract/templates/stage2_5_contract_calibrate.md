...
本阶段解决的根因：**测试参考对响应结构的臆测会导致整批用例同错**。常见臆测坑（通用归纳，具体协议示例见 workers/Stage4-TestGenerationRun/examples/a2a/）：
...
- **不要硬编码任何特定协议**；按 SUT 实际协议/端点描述（具体协议形态示例见 workers/Stage4-TestGenerationRun/examples/a2a/）
...
1. `{skill_dir}/workers/_common/shared/scenario_schema.md` — schema
...
```bash
python3 {skill_dir}/workers/Stage2-CodeAnalysisContract/scripts/probe_contract.py \
  --target-id {target_id} \
...
> 上表 specId 为通用骨架；具体协议（如 A2A 的 result.task / TASK_STATE_* / SSE oneof / card.url）的填写示例见 workers/Stage4-TestGenerationRun/examples/a2a/。
```
