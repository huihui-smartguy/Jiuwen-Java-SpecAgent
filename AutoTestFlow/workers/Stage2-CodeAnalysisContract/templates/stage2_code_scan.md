...
```bash
python3 {skill_dir}/workers/Stage2-CodeAnalysisContract/scripts/prepare_code_scan.py --code-path {code_path} --output-dir {output_dir}
```

然后读取：

1. `{output_dir}/FeatureAnalysis/code_scan_plan.json` — profile 选择、证据、P0/P1/P2 搜索 hints
2. `{skill_dir}/workers/Stage2-CodeAnalysisContract/shared/code_scan_profiles.json` — adapter profile 定义
...
1. `{skill_dir}/workers/_common/shared/scenario_schema.md` — 场景 JSON schema + code_facts schema
2. `{skill_dir}/workers/Stage2-CodeAnalysisContract/shared/code_scan_guide.md` — **通用扫描方法学**（adapter layer、稳定 fact model、自检）
3. `{skill_dir}/workers/Stage2-CodeAnalysisContract/shared/code_analysis_template.md` — 代码分析报告格式

仅当 `primary_profile == "java.spring"` 时读取：

4. `{skill_dir}/workers/Stage2-CodeAnalysisContract/shared/java_scan_guide.md` — Java/Spring profile 附录
