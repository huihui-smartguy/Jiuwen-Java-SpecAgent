...
> **核心目标**：复制 P0 已验证的代码结构保持一致性；断言独立按 contract.md 设计；同样门禁 + **5分类失败处理** + 轨迹
...
1. **`{target_output_dir}/contract.md`** — **唯一权威 oracle**，本用例断言期望值取自此处对应 specId。
...
4. `{target_output_dir}/.state/skeleton/`（如有）。
5. `{target_output_dir}/.state/stage_summary.json`（如有，含 user_test_entry / forbidden_direct_apis）。

### 第二步：确认复用资产已就位

`{target_output_dir}/http_client.py` 与 `conftest.py` 应已由 P0 阶段复制到位；如缺失则补：
...
```bash
python3 - <<'PY'
import sys; sys.path.insert(0, "{target_output_dir}")
...
```bash
python {validate_script} {target_output_dir}/test_{case_id_lower}.py
...
轨迹自动落 `{target_output_dir}/.state/trace/{test_name}.jsonl`。

### 第六步：5分类失败处理 + 有界自我修复循环（最多 {max_fix_attempts} 轮）

pytest 通过即 **pass**。失败/报错时先判定归类（5 类）：
...
#### 7a. 写入 `{target_output_dir}/.state/results/{case_id}.json`
...
> status=passed 时 class=null、skip_reason/sdk_defect 省略。
