...
### 第3.5步（条件激活）：故障库驱动的用例补充

> 仅当 `{output_dir}/.state/fault_matches.json` 存在时执行（由阶段2.6 `match_faults.py` 产出）；不存在则跳过，本步对产物零影响（与未接入故障库时一致）。
...
- 新增字段 **`fault_ref`** = 匹配项 `fault_id`（用例溯源标记；下游 merge 原样透传，stage5 据此统计故障覆盖）。
...
- **`fault_ref`（可选）**：故障导向用例专用，值为故障库 `fault_id`（如 `F-REQ-011`），来自第3.5步；普通用例不含此字段。下游 `merge_test_design.py` 原样透传，stage5 据此统计故障库覆盖。
