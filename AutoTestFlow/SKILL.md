...
| 模式 | 触发条件 | 流程 | 输出 |
|------|----------|------|------|
| **标准模式** | 提供 `code_path` / `--pr` / `--commit` 之一 | 0(可选)→1→2→**2.5**→3a→3b→4a→4b→5→[6→确认门→7](可选,门控) | 真实契约校准 + 可执行黑盒测试 + 报告 +(可选)fault analysis + 本地复验 + evidence issue |
 ...
 行│                   │
  ...
  阶段2.5 契约校准（关键门）：
    probe_contract.py --base-url {sut} → contract_samples.json
      ├─ SUT 可达 → 采样真实响应 + 结合 stage2 源码事实 → contract.md（权威 Oracle）
...
| `code_path` | 被测源码目录（stage2 自动识别 Java/Spring、Python Web/API、C++ service/RPC 或 fallback） | **可选**（与 --pr/--commit 三选一，均未提供则纯需求模式） |
...
步骤2: 子Agent 产出 contract.md（templates/stage2_5_contract_calibrate.md）
  → 输入: .state/contract_samples.json + .state/s2_code_facts.json（源码事实）
...
└── .state/
    ...
    ├── s2_code_facts.json
    ├── code_scan_plan.json       # 阶段2（profile adapter 预扫描计划）★
    ├── framework_scenes.json     # 阶段2（从源码结构派生的框架 E2E 场景，stage3a-fw 输入）★
    ...
    ├── fault_analysis/           # 阶段6（可选,门控）★ analysis_plan/manifest + targets/<id>/{root_cause,evidence,fix_solution,issue,confidence}
    └── remediation/              # 阶段6/7 patchable 子集 ★ plan/manifest/reverify/submitted/issue_bodies + defects/<case>/{root_cause,evidence,fix_solution,patch.diff,regression_test.diff,issue.md,confidence.json}
