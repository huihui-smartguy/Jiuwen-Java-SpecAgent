...
stage1 与 stage3b 的裁决是**强制门**：阶段完成后**必须**调用 `AskUserQuestion`，得到用户确认才进入下一阶段。规格库就位后，这两个门可降级为自动通过。
...
| 3a-fw 框架场景 | `templates/stage3a_framework.md` | `reference/client_reference.md` | `.state/s3a_framework.json` | ✅ | ❌ |
...
步骤2: 2-Agent并行启动（run_in_background=True）
  ...
  → Agent-S3a-fw（框架场景补充）
    ...
    输入: .state/s1_index.json + reference/client_reference.md + .state/stage_summary.json
