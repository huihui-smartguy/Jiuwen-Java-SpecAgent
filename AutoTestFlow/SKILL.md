...
> 标准模式先用 Stage 0 LLM/编排解析自然语言 `autotestflow.suts.md`，再由 `scripts/sut_manifest.py` 确定性校验并归一化为 `.state/sut_manifest.normalized.json`；legacy Markdown+YAML manifest 仍直接走同一校验器。每个 target 在自己的 `targets/<target_id>/` 下执行阶段2→5，并在阶段2之后**强制插入阶段2.5「契约校准」**，产出 target-local 权威 Oracle `contract.md`。纯需求模式无代码亦无运行服务，断言点统一标 `needs-runtime-verify`。
...
标准模式（multi-SUT）：
  阶段0：sut_manifest.py --sut-manifest {path} --write
    → .state/sut_description.parse.json
    → .state/sut_description.review.md
...
步骤2: 子Agent 产出 contract.md（templates/stage2_5_contract_calibrate.md）
  → 输入: {target_output_dir}/.state/contract_samples.json + {target_output_dir}/.state/s2_code_facts.json（源码事实；source unavailable 时可缺省/占位）
