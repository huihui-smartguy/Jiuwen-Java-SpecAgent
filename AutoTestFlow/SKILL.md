...
| 模式 | 触发条件 | 流程 | 输出 |
|------|----------|------|------|
| **标准模式** | 提供 `--sut-manifest`，或 legacy `code_path`/`--pr`/`--commit` + `--sut-base-url` | 0 manifest 归一化→1→target loop(2→**2.5**→3a→3b→4a→4b→5)→root report→[6→确认门→7](默认门控) | 每个 target 独立真实契约校准 + 可执行黑盒测试 + target 报告 + root 汇总 + 默认门控 fault analysis + 本地复验 + evidence issue |
...
| 2.6 知识/故障匹配（默认） | —（纯脚本） | `scripts/match_faults.py` | `.state/knowledge_matches.json` + `.state/fault_matches.json` + `.state/fault_contract_alignment.md` | — | ❌ |
| 2.6b 故障增强（默认按需） | `templates/stage2_6_fault_match.md` | — | 回写 `.state/fault_matches.json` | ✅ | ❌ |
...
| 7 应用-构建-复验-evidence issue（默认门控） | —（纯脚本） | `scripts/apply_and_reverify.py` + `scripts/submit_remediation.py` | `.state/remediation/{reverify.json,submitted.json,issue_bodies/<case>.md}` | ❌ | ❌ |
...
步骤1: 编排器前景执行（不启动 Agent）
  ...
  → 输出: .state/knowledge_matches.json（主产物）+ .state/fault_matches.json（兼容产物，含 fault_ref/branch_class/oracle_refs/fault_oracles/reconciliation）

步骤2: 验证
  → 若 .state/fault_matches.json 存在 → stage3b 将据此补充故障导向用例，并原样保留 required fault_oracles
...
步骤2: 4a P0深度验证（max 2并发，run_in_background=True）
  ...
  → 对每个P0用例：
    ...
    输出: {target_output_dir}/test_{case_id}.py + .state/results/{case_id}.json（含 fault_oracle_summary）+ .state/trace/{case_id}.jsonl

步骤3: P0质量抽检（编排器轻量执行）
  → 对每个P0用例读取 .state/results/{case_id}.json（只读 status/error_class/trace_file/assert_level/fault_oracle_summary）
...
步骤5: Python脚本（前景，<1s）
  ...
  → 输出: case_results.json（每条含执行分类，fault_ref 用例保留 fault_oracle_summary / fault_oracle_results）
...
│       ├── case_results.json     # target 阶段4（含执行分类与 fault_oracle 汇总）
