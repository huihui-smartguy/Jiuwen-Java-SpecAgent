...
| 模式 | 触发条件 | 流程 | 输出 |
|------|----------|------|------|
| **标准模式** | 提供 `--sut-manifest`，或 legacy `code_path`/`--pr`/`--commit` + `--sut-base-url` | 0 manifest 归一化→1→target loop(2→**2.5**→3a→3b→4a→4b→5)→root report→[6→确认门→7](可选,门控) | 每个 target 独立真实契约校准 + 可执行黑盒测试 + target 报告 + root 汇总 +(可选)fault analysis + 本地复验 + evidence issue |
| **纯需求模式** | 以上均未提供 | 1→2R→3aR→3b（终止） | 测试设计文档 |

> 标准模式先用 `scripts/sut_manifest.py` 把 Markdown+YAML manifest 归一化为 `.state/sut_manifest.normalized.json`。每个 target 在自己的 `targets/<target_id>/` 下执行阶段2→5，并在阶段2之后**强制插入阶段2.5「契约校准」**，产出 target-local 权威 Oracle `contract.md`。纯需求模式无代码亦无运行服务，断言点统一标 `needs-runtime-verify`。
...
标准模式（multi-SUT）：
 ...
 行│                   │
  ...
  target 阶段2.5 契约校准（关键门）：
    probe_contract.py --target-id {target_id} --base-url {sut_base_url} [--probe-plan {probe_plan}] → targets/<target_id>/.state/contract_samples.json
      ├─ target 可达 → 采样真实响应 + 结合 target stage2 源码事实 → targets/<target_id>/contract.md（权威 Oracle）
...
| `--sut-manifest` | Markdown + YAML SUT manifest，标准模式主入口；支持多个 target、源码路径、运行方式、readiness、probe plan、依赖与 remediation 配置 | **标准模式推荐必填** |
...
| `--remediate` | fault analysis / 自动修复总开关（阶段6/7）：`off`(不产任何修复文件,流水线字节级一致) / `dry-run`(分析+本地复验,**不**提交 issue) / `on`(经**强制人工确认门**后提交 evidence issue；不再提交 PR) | `off` |
| `--remediation-config` | legacy 单 target 修复配置；multi-SUT 优先使用 manifest target 的 `remediation.config_path` 或 `targets/<target_id>/remediation.config.json` | legacy 探测 `<output_dir>/remediation.config.json` → `<work_dir>/.autotestflow/remediation.config.json` |
...
**注意**：
- `--sut-manifest` 是标准模式主入口；`--sut-base-url` 只保留为旧调用兼容 shim，并会输出 deprecation note。
...
| 框架场景（迭代6 外部 `e2e_framework_scenes.md`） | **框架场景由 stage2 从源码结构自动派生** → `.state/framework_scenes.json`（**纯需求模式由 stage3aR 从需求侧派生**） |
...
| 2 通用代码扫描 | `templates/stage2_code_scan.md` | `scripts/prepare_code_scan.py` + `shared/code_scan_profiles.json` + `shared/code_scan_guide.md` + `shared/code_analysis_template.md` | `.state/code_scan_plan.json` + `.state/s2_code_facts.json` + `.state/stage_summary.json` + `.state/framework_scenes.json`（框架场景派生物） | ✅ | ❌ |
| 2.5 契约校准 | `templates/stage2_5_contract_calibrate.md` | `scripts/probe_contract.py` | `targets/<target_id>/contract.md` + target `.state/contract_samples.json` | — | ❌ |
...
| 6 fault analysis（可选,门控） | `templates/stage6_fault_analyze.md` + `templates/stage6_defect_analyze.md` | `scripts/remediation_plan.py` + `shared/remediation_rules.md` + `shared/fault_analysis_profiles.json` + `reference/remediation_config.py` | `.state/fault_analysis/{analysis_plan.json,manifest.json,targets/<id>/*}` + `.state/remediation/{plan.json,manifest.json,defects/<case>/*}` | ✅(逐目标) | **✅(强制,仅 on)** |
| 7 应用-构建-复验-evidence issue（可选,门控） | —（纯脚本） | `scripts/apply_and_reverify.py` + `scripts/submit_remediation.py` | `.state/remediation/{reverify.json,submitted.json,issue_bodies/<case>.md}` | ❌ | ❌ |
...
步骤1: 并行启动（run_in_background=True）
  ...
  → 编排器前景执行 stage2 profile 预扫描（不启动 Agent）
    ...
    输出: .state/code_scan_plan.json（primary_profile/language/frameworks/confidence/scan_hints）

  → Agent-S2（通用代码事实扫描）
    模板: templates/stage2_code_scan.md
    参数: {skill_dir} {output_dir} {code_path}
    输出: .state/s2_code_facts.json + .state/stage_summary.json（简化版）+ .state/framework_scenes.json
    子Agent内部读取 .state/code_scan_plan.json + shared/code_scan_guide.md + shared/code_scan_profiles.json，
...
步骤2: 子Agent 产出 contract.md（templates/stage2_5_contract_calibrate.md）
  → 输入: {target_output_dir}/.state/contract_samples.json + {target_output_dir}/.state/s2_code_facts.json（源码事实）
  → 产出 {target_output_dir}/contract.md：真实响应包装/字段/枚举/错误码/事件形态
...
步骤0: SUT 就绪门（编排器前景执行）
  → 执行: python {skill_dir}/scripts/sut_runtime.py \
            --manifest-normalized {output_dir}/.state/sut_manifest.normalized.json \
  ...
  → if reachable == false:
      ...
      → 写 {target_output_dir}/case_results.json（全部 env_issue + note: target 未就绪）
...
步骤1: Python脚本（前景，<1s）
  ...
  → 输入: target-local test_design.json + .state/stage_summary.json + contract.md
...
步骤2: 4a P0深度验证（max 2并发，run_in_background=True）
  ...
  → 对每个P0用例：
    ...
    输出: {target_output_dir}/test_{case_id}.py + .state/results/{case_id}.json + .state/trace/{case_id}.jsonl
...
### 阶段6：fault analysis（可选，门控；每个分析目标一个子Agent）

> 人工裁决：✅（强制确认门，仅 `--remediate=on`）| 条件：标准模式 + `--remediate≠off` + 配置就位 + 存在可分析失败/故障目标
...
步骤1: 编排器前景执行 analysis plan（不启动 Agent）
  ...
  → 产 .state/fault_analysis/analysis_plan.json（所有分析目标）
  → 同时产 .state/remediation/plan.json（仅 patchable_contract_defect，供 stage7 apply/reverify 兼容）
...
步骤2: 逐目标启动子Agent（run_in_background=True，≤ case-batch-size）
  ...
  → 产出 .state/fault_analysis/targets/{target_id}/{root_cause.md,evidence.json,fix_solution.md,issue.md,confidence.json}
  → patchable 目标额外产出 .state/remediation/defects/{case_id}/{root_cause.md,evidence.json,fix_solution.md,patch.diff?,regression_test.diff?,issue.md,confidence.json}
...
步骤3: 汇总 manifest（前景）
  ...
  → .state/fault_analysis/manifest.json + .state/remediation/manifest.json（后者仅 patchable 子集）
...
### 阶段7：应用 + 构建 + 复验 + evidence issue（可选，门控，纯脚本）
...
步骤2: evidence issue 提交（仅 --remediate=on 且门确认；dry-run/取消 → 零 gh 调用）
  ...
  → 生成 .state/remediation/issue_bodies/{case}.md（stage6 issue + fix_solution + patch 摘要 + reverify 实证）
  → gh issue create (upstream, --body-file issue_bodies/{case}.md)；幂等：先 list 去重复用 URL
  → .state/remediation/submitted.json（issues / skipped；无 pr 字段）
...
├── targets/
│   └── {target_id}/
│       ├── code_analysis.md      # target 阶段2（摘要）
│       ├── contract.md           # target 阶段2.5（真实契约，权威 Oracle）★
│       ├── test_design.json      # target 阶段3b（Python merge）
│       ├── scene_tc_mapping.json # target 阶段3b
│       ├── case_results.json     # target 阶段4（含 error_class 5分类）
│       ├── test_{case_id}.py     # target 阶段4（Python pytest + httpx 黑盒）
│       ├── report.md             # target 阶段5（含轨迹区）
│       └── .state/
│           ├── code_scan_plan.json
│           ├── s2_code_facts.json
...
└── .state/
    ...
    ├── fault_analysis/           # root 汇总索引；target 明细仍落在 targets/<target_id>/.state/...
    └── remediation/              # root 汇总索引；patch/reverify 产物 target-local
...
│   ├── sut_manifest.py                   # 阶段0 Markdown+YAML manifest 解析 / legacy 单 target 归一化 ★
...
- 阶段6（可选,门控）：`--remediate≠off`+配置+可分析目标触发；`.state/fault_analysis/analysis_plan.json` 覆盖 patchable/issue-only/diagnostic 目标，`.state/remediation/plan.json` 仅含 patchable contract defect | 每目标产 evidence/root_cause/fix_solution/issue/confidence，只有 spec-required+可定位目标产 patch/selftest | manifest 就绪后 **`--remediate=on` 强制人工确认门** | 定位不到→needs_human、仅记 evidence issue 不强行 patch | **不弱化测试/不改 contract.md**
- 阶段7（可选,门控）：绿由 `apply_and_reverify.py` 重跑未改动用例+重建SUT 判定（非LLM）| 构建失败/仍红→写入 reverify 证据、不抛栈 | 提交仅 `on`+过门+`switches.allow_open_issue`+实证前提 | 只创建 upstream evidence issue，幂等去重；不推送 fork、不创建 PR | dry-run/取消→零 gh 调用 | clone仓与轨迹 gitignore
