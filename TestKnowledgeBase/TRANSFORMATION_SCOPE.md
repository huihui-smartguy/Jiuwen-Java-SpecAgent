...
|---|---|---|
| REST/API fault patterns | `TestKnowledgeBase/Fault/rest_api_faults.json` | Default AutoTestFlow runtime source. |
| Web, Agent, and DFX fault patterns | `TestKnowledgeBase/Fault/*.json` | Expanded by domain without changing `contract.md` oracle rules. |
...
1. AutoTestFlow must prefer `TestKnowledgeBase/Fault/rest_api_faults.json` when `--fault-lib` is not supplied.
2. A fallback to `Specification_Repository/rest_api_common_faults.json` is allowed only to keep old demos and old callers from breaking.
