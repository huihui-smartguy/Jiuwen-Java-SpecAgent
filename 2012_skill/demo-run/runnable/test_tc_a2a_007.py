...
def _event_task_id(ev):
    """容差从 SSE 事件载荷读取 taskId（兼容 id / taskId / task.id 字段位置）。"""
    ...
    if isinstance(status, dict) and status.get("taskId"):
...
def test_tc_a2a_007(a2a_client):
    ...
    for ev in stream:
        ...
        if task_id:
            break
    assert task_id, "期望从早期 SSE 事件获取到 taskId（用于取消执行中任务）"

    # Act —— 取消执行中的任务
    cancel_resp = a2a_client.cancel_task(task_id, req_id="7")

    # Assert —— 过程维 (process dim)：id 回带
    assert cancel_resp.get("id") == "7", \
    ...
    state = normalize_state(cancel_resp["result"]["status"]["state"])
