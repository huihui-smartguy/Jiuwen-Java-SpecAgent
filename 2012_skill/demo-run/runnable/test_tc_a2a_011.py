...
def test_tc_a2a_011(a2a_client):
    ...
    @Step: 1. GET agent-card.json 读取声明的 endpoint（缺省回退 /a2a）
           ...
           3. 校验返回 Task 可达且终态
    @Result: Card endpoint 含 /a2a；据此 SendMessage 返回 Task(有 id 与 status.state)，state ∈ TERMINAL_STATES(COMPLETED)
    """
    # Arrange —— 读取 Agent Card，解析声明的端点
    card = a2a_client.get_agent_card()
    assert isinstance(card, dict), f"期望 AgentCard 为 dict，实际 {type(card).__name__}"
    endpoint = card.get("url") or card.get("endpoint") or A2A_ENDPOINT  # 缺省回退 /a2a

    # Assert —— 过程维 (process dim)：发现-调用一致性，声明端点含 /a2a
    assert A2A_ENDPOINT in endpoint, \
        f"期望 Card endpoint 含 {A2A_ENDPOINT!r}，实际 {endpoint!r}"

    # Act —— 据发现的端点发起调用（client 即对准 base_url + /a2a）
    send_resp = a2a_client.send_message(
        "你好", context_id="ctx-discover-011", message_id="msg-discover-011", req_id="11")

    # 结构存在性 (L1) + id 回带（过程维补充）
    assert send_resp.get("id") == "11", f"期望 id 回带 '11'，实际 {send_resp.get('id')!r}"
    assert "result" in send_resp, f"期望成功返回 Task，实际 keys={list(send_resp)}"
    task = send_resp["result"]
    assert task.get("id"), f"期望返回 Task 含 id，实际 result keys={list(task)}"

    # Assert —— 内容维 (content dim, L2)：据 card endpoint 调用得到终态 COMPLETED
