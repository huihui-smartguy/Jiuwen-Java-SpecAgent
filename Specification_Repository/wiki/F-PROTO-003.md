# F-PROTO-003 · JSON-RPC方法不存在

> 溯源：`rest_api_common_faults.json` → `FC-PROTO-001`（协议合规性缺陷）→ `F-PROTO-003`
> 严重程度：高 ｜ 故障建议断言级别：L2（**最终以项目 `contract.md` 权威性封顶**，见下「契约锚点」）
> 标签：协议合规、JSON-RPC、异常输入
> 本文由 `beta/scripts/gen_wiki.py` 从结构化故障库**单向派生**，仅作 advisory；**不作 oracle、不进 `match_faults.py`**。

## 故障模式（通俗描述）

调用未定义的method应返回method not found错误
