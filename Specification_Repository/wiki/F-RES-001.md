# F-RES-001 · 响应id字段类型不一致

> 溯源：`rest_api_common_faults.json` → `FC-RES-001`（响应体缺陷）→ `F-RES-001`
...
响应中的id字段类型与请求不一致（如请求string，响应int）
