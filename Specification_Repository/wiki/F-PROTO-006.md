# F-PROTO-006 · Content-Type错误

> 溯源：`rest_api_common_faults.json` → `FC-PROTO-001`（协议合规性缺陷）→ `F-PROTO-006`
> 严重程度：中 ｜ 故障建议断言级别：L2（**最终以项目 `contract.md` 权威性封顶**，见下「契约锚点」）
> 标签：HTTP协议、Content-Type
> 本文由 `beta/scripts/gen_wiki.py` 从结构化故障库**单向派生**，仅作 advisory；**不作 oracle、不进 `match_faults.py`**。

## 故障模式（通俗描述）

发送错误的Content-Type（如text/plain代替application/json）

## 怎么触发

发送Content-Type为text/plain的JSON请求
